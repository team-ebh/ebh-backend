...
<head>
    ...
    <title>Customer Private Channel Test</title>
...
<div class="container mx-auto px-4 py-8 max-w-7xl">
    ...
    <div class="bg-gradient-to-r from-blue-600 to-blue-700 rounded-lg shadow-lg p-6 mb-6 text-white">
        <h1 class="text-3xl font-bold mb-2">👤 Customer Private Channel Test</h1>
        <p class="text-blue-100">Test private channel authentication and real-time events</p>
    </div>

    <!-- Login Section -->
    <div class="bg-white rounded-lg shadow-lg p-6 mb-6">
        <h2 class="text-xl font-bold text-gray-800 mb-4">1️⃣ Login as Customer</h2>
        ...
        <div class="flex items-center justify-between">
            <div>
                <p class="text-sm text-gray-600 mb-2">
                    <strong>Channel:</strong> <code id="channel-name" class="bg-gray-100 px-2 py-1 rounded">customer.{id}</code>
                </p>
            ...
            <div class="flex items-center gap-3">
                <button
                    id="connect-btn"
                    class="px-6 py-2 bg-blue-600 text-white font-semibold rounded-lg hover:bg-blue-700 transition disabled:bg-gray-400"
                    disabled
                >
                    Connect
                </button>
...
<script>
    ...
    verifyBtn.addEventListener('click', async () => {
        ...
        try {
            ...
            if (response.ok && data.data?.token) {
                ...
                authStatus.innerHTML = `<span class="text-green-600">✓ Authenticated as Customer #${customerId}</span>`;
                channelName.textContent = `customer.${customerId}`;
    ...
    connectBtn.addEventListener('click', () => {
        console.log('Connect clicked. Token:', token, 'Customer ID:', customerId);

        if (!token || !customerId) {
            log('error', 'Please authenticate first', {token: !!token, customerId: !!customerId});
            return;
        }

        log('info', 'Connecting to private channel...', {channel: `customer.${customerId}`});
        ...
        channel = echo.private(`customer.${customerId}`);

        channel.subscribed(() => {
            log('success', 'Connected to private channel!', {channel: `customer.${customerId}`});
