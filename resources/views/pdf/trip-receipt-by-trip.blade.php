...
@php
    $detailsRows = [
        ...
        [
            ...
            'value' => $trip->{Trip::COLUMN_UPDATED_AT}->format('M d, Y H:i')
        ],
        [
    ...
    // Add payment status only if payment exists
    if ($hasPayment) {
        $detailsRows[] = [
        ...
        $detailsRows[] = [
            'label' => trans('receipts.payment_status'),
            'value' => $isCashPayment ? trans('receipts.cash_payment') : '-'
        ];
    }
@endphp
...
<div class="section-title">{{ trans('receipts.trip_summary') }}</div>
...
<table>
    ...
    <tr style="background-color: #f4f4f4; font-weight: bold;">
        <td>{{ trans('receipts.total') }}</td>
        <td>{{ priceFormat($trip->{Trip::COLUMN_TOTAL_PRICE}) }} {{ $trip->{Trip::COLUMN_CURRENCY}?->getLabel() }}</td>
    </tr>
    </tbody>
</table>
